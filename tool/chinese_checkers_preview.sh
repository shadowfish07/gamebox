...
readonly log_file="$(mktemp "${TMPDIR:-/tmp}/gamebox-chinese-checkers-preview.log.XXXXXX")"
trap 'rm -f "$log_file"' EXIT

command=(
	"$GODOT_BIN"
	--path "$ROOT/game_runtime"
	--script "$ROOT/tool/chinese_checkers_preview.gd"
...
if (( status != 0 )); then
	printf 'FAIL chinese-checkers-preview (Godot render)\n' >&2
...
printf 'PASS chinese-checkers-preview (artifact=%s, warnings=%s)\n' "$artifact" "$warning_count"
