...
func TestDurableTwoPlayerMatchHappyPath(t *testing.T) {
	...
	for index, move := range winningMoves {
		...
		assertSameMoveEvent(t, aliceEvent, bobEvent, revision+1, move.userID, move.x, move.y)
	}
...
func TestDurableMatchRestoresAfterCompleteServerAndDatabaseRestart(t *testing.T) {
	...
	for index, move := range preRestartMoves {
		...
		assertSameMoveEvent(t, mustReadEnvelope(t, ctx, aliceWS), mustReadEnvelope(t, ctx, bobWS), int64(index+1), move.userID, move.x, move.y)
	}
	beforeRestart := requestSnapshot(t, ctx, aliceWS, created.ID, 3)
	if beforeRestart.Revision != 3 {
		t.Fatalf("pre-restart revision=%d", beforeRestart.Revision)
	}
	...
	for index, move := range postRestartMoves {
		...
		assertSameMoveEvent(t, mustReadEnvelope(t, ctx, aliceAfterRestart), mustReadEnvelope(t, ctx, bobAfterRestart), revision+1, move.userID, move.x, move.y)
	}
...
END`); err != nil {
	...
	noBroadcastContext, stopNoBroadcast := context.WithTimeout(context.Background(), 150*time.Millisecond)
	defer stopNoBroadcast()
	if unexpected, readErr := peer.ReadEnvelope(noBroadcastContext); readErr == nil {
		t.Fatalf("peer received uncommitted event: %+v", unexpected)
	}

	unchanged := requestSnapshot(t, ctx, actor, created.ID, 0)
	if unchanged.Revision != 0 || unchanged.Status != "active" {
		t.Fatalf("snapshot changed after failed commit: %+v", unchanged)
	}
	for index, cell := range unchanged.Board {
		if cell != 0 {
			t.Fatalf("board[%d]=%d after failed commit", index, cell)
		}
	}
...
func assertSameMoveEvent(t *testing.T, left, right protocol.Envelope, revision int64, userID string, x, y int) {
	t.Helper()
	if left.Type != protocol.TypeGomokuMoveAccepted || right.Type != protocol.TypeGomokuMoveAccepted || left.GameID != gomoku.GameID || right.GameID != gomoku.GameID || left.MatchID == "" || left.MatchID != right.MatchID || left.ActionID == "" || left.ActionID != right.ActionID || left.Revision == nil || right.Revision == nil || *left.Revision != revision || *right.Revision != revision || !bytes.Equal(left.Payload, right.Payload) {
		t.Fatalf("move broadcasts differ: left=%+v right=%+v", left, right)
	}
	var payload struct {
		X      int    `json:"x"`
		Y      int    `json:"y"`
	...
	if err := json.Unmarshal(left.Payload, &payload); err != nil || payload.X != x || payload.Y != y || (payload.Color != "black" && payload.Color != "white") || payload.UserID != userID {
		t.Fatalf("move payload=%s decoded=%+v err=%v", left.Payload, payload, err)
	}
}
