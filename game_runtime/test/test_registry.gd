...
static func cases() -> Array:
	return [
		...
		{"name": "registry resolves flight chess to a packed scene", "run": _resolves_flight_chess_scene},
...
static func _resolves_flight_chess_scene() -> bool:
	var result: Dictionary = GameRegistry.resolve("flight_chess")
	return _check(result.get("ok", false), "expected flight chess to resolve") \
		and _check(result.get("scene") is PackedScene, "expected a PackedScene")
...
static func _contains_supported_games() -> bool:
	...
	return _check(game_ids.size() == 4, "expected exactly four registered games") \
		and _check(game_ids.has("chinese_checkers"), "expected chinese checkers to be registered") \
		and _check(game_ids.has("flight_chess"), "expected flight chess to be registered") \
