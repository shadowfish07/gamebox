...
static func setup(scene: Control) -> void:
	...
	for card_name in ["OpponentCard", "LocalCard"]:
		...
		capture.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
...
static func layout(scene: Control, regions: Dictionary, dark: bool) -> void:
	...
	for pair in [["OpponentCard", 56], ["LocalCard",56+card_height+card_gap]]:
		...
		capture.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
		capture.add_theme_font_size_override("font_size", roundi(Tokens.TYPOGRAPHY.label_small.font_size*unit))
		capture.add_theme_color_override("font_color", colors.on_surface_variant)
...
static func present_card(scene: Control, node_name: String, color: String, active: bool, presence: String, pieces: Array, capture_count: int = 0) -> void:
	...
	card.get_node("Content/Name/CaptureCount").text = "撞回 %s 架" % (str(capture_count) if capture_count >= 0 else "—")
