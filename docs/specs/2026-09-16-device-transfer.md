...
- `POST /v1/auth/transfer/redeem`：公开请求 `{code, receiver}`；返回 `{session, snapshot}`。输入忽略大小写和空格。恢复未完成的迁出时附加 `expectedUserId` 查询参数，服务端在消费码或回放回执前核对原账号，账号不匹配不消费码。`receiver` 是新设备提前写入安全存储的 256 位随机重试秘密。
...
新设备恢复本地存档、写入 refresh token、清除迁入日志后才发布已登录状态并启动大厅/收藏同步。网络或存储失败保留日志；重启先恢复迁移。旧设备生成前持久记录含原账号 ID 的取消义务，重启先取消旧快照再允许继续使用，防止旧码携带过时抽卡进度。若凭据永久失效，恢复页仍可使用现有迁移码/管理员恢复码流程；只有同一原账号的替代存档与会话持久化后才解除旧迁出锁。日志读取恢复成功后退出读取重试状态，由迁出恢复页继续保护原快照并提供恢复入口。收到服务端确认的账号迁移后，旧设备先排空收藏写入并清理本地副本，再清理凭据、开放注册；清理失败保留凭据以便重试。
