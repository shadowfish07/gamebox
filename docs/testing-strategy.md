...
- `bash tool/verify.sh` 是代码任务交付前的顶层确定性 gate；它不代表已运行实际
  Android 设备或双设备业务 E2E。Android 单元测试、APK 构建和 APK 打包断言不属于
  PR gate，由 tag 发布工作流通过 `tool/verify_android_apk.sh` 执行。
...
| 单一 Android 宿主或打包边界 | 聚焦构建、`tool/verify_android_apk.sh` 打包断言、安装、启动和 host smoke | 只有网络、协议、共享权威状态或多玩家行为变更时升级为双设备 E2E |
