...
| MOVE-11 超出终点 | `roll.accepted` 可移集合计算 | home 飞机需要的点数小于本次 roll | 用户尝试点击该飞机 | 该飞机不高亮，点击不提交 | 无 pending | 不存在反弹/倒退 | 若过期 UI 仍提交，服务返回 invalid_move，恢复可选集合 | R + I。`REC P1`：可用一次轻量规则提示解释“需要精确点数”，不为每次无效点击弹出 snackbar。 |
