...
| M7 | **Required state and visual acceptance evidence is missing.** Current Widget/headless tests cover useful wiring, but not the complete light/dark, normal-size content-growth, safe-area, confirmation, interaction-state, and visual-review gates. The fixed E2E harness intentionally records logic assertions only and does not capture runtime screenshots. | Pass focused component/flow tests, the repository gate, and the real two-device linked logic flow on the final clean HEAD. Keep any visual screenshot review as a separate target-runtime activity; fake services remain wiring evidence only. |
...
5. With the repository's shared Android lease, run the actual built App and packaged Godot game on `Gamebox_A_API_36` and `Gamebox_B_API_36`; drive the real two-device flow with `bash tool/e2e_android.sh`. The fixed E2E asserts logs, UI state, lifecycle, and authoritative snapshots only; it intentionally does not capture or compare pixels. Deterministic fake services prove UI/state wiring, not the live WebSocket/server boundary.
...
7. If the UI acceptance for the retrofit requires visual evidence, capture every row in the screenshot matrix from the actual built runtime using safe scripted identities and inspect each PNG for clipping, state accuracy, and sensitive data. This is separate from the fixed logic-only E2E and must not be added back as a screenshot gate inside `tool/e2e_android.sh`.
