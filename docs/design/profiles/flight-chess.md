...
- 主路径是 52 格闭环，四方主路径起点相隔 13 格；每色从公共归家入口再走 6 步到达终点：5 个独立航道方格，第 6 格为中央终点。
- 公共格按蓝、黄、绿、红循环；每色归家入口与长距离飞跃触发点都落在本色格，避免把起飞区误当作同色公共格。
- 四条长距离飞跃保持黄向下、绿向左、红向上、蓝向右的经典方向。
- 玩家先请求服务端掷骰子，再手动选择一架可移动的飞机。掷出 5 或 6 可从机库起飞，飞机停在起飞位；5 起飞后结束回合，6 起飞后获得一次额外掷骰。
- 捷径飞行经过对方归家航道第 3 格时，撞回该交叉格上的全部敌机，然后继续飞到捷径终点；相邻归家格、普通走子和四格跳跃不触发此撞机。
...
生产验收覆盖大厅创建/续局、双端启动、服务器离线时的 pending 边界、权威掷骰与移动、额外回合、强停后恢复、非破坏性返回、认输与结果。UI 运行时检查 `ready`、`rolled`、`pressed`、`selected`、`stacked`、`offline/pending`、`resign-confirmation` 和 `result` 状态，至少覆盖 960×540 窄横屏和 2400×1080 的 20:9 横屏，并在浅色、深色背景下确认固定棋盘艺术仍然可读。布局读取 Android 可用安全区域；确定性预览的 `--safe-insets LEFT,TOP,RIGHT,BOTTOM` 使用实际窗口像素，再换算到当前虚拟像素，用于模拟横屏挖孔和手势边缘。右侧掷骰操作固定在拇指容易触达的底部区域；棋盘选择为每架可移动飞机提供至少 48dp 的容错直径，触摸按下先缩放反馈，释放后选中并预览路线，再点“确认移动”提交；可取消或换选，点得太远则不会误选。叠放飞机合并为一个选择目标，通过层叠边缘和数量角标表达组内数量，点击后明确选择组内编号，避免多套高亮互相干扰。

2026-09-05 的纯 Godot / HTML 对照验收见 [验收记录](../2026-09-05-flight-chess-html-godot-acceptance.md)。2026-09-06 采用用户选定的 B 方案：生产 HUD 仅显示“已抵达 n / 4”和四架飞机，未抵达为空心、抵达后按阵营色填充，四架完成时显示“全部抵达”；不再显示在途与机库数量，不显示示例昵称；认输收进更多菜单。权威移动依次表现逐格走子、跳跃/捷径、撞击和整组回库，精确抵达后棋子缩小淡出；超点沿归家轨道折返。动效结束后更新回合卡和计数，最后一架收起后显示结果。连续事件排队，重连/快照中断旧动效，以权威状态恢复。
