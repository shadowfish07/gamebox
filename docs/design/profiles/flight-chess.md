...
2026-09-05 的纯 Godot / HTML 对照验收见 [验收记录](../2026-09-05-flight-chess-html-godot-acceptance.md)。生产 HUD 使用在途、机库、抵达图标加数字，不显示示例昵称；认输收进更多菜单。权威移动依次表现逐格走子、跳跃/捷径、撞击和整组回库，精确抵达后棋子缩小淡出；超点沿归家轨道折返。动效结束后更新回合卡和计数，最后一架收起后显示结果。连续事件排队，重连/快照中断旧动效，以权威状态恢复。
