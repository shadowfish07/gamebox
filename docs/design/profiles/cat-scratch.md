...
采用 tokens 2.5.0 的 `game.scratch*` 美术角色。短屏职业证将线索与照片并排，保持票面文字和触摸区域可用。

刮奖页以票面为主体，底部固定揭晓/再来一张按钮和三项导航。涂层是连续触摸画布，普通控件保持标准触摸目标。职业证先线索后照片，四爪印需全部完成。部分揭晓不暴露结果文案，完全揭晓后显示新收藏或重复数。

可见返回和 Android Back 共用保存状态检查。正常返回保留收藏；存储错误时提示重试，明确确认后才可放弃未保存进度。结果内联显示，详情和规则用可关闭底部弹层。图鉴与六格展柜允许纵向滚动。

验收状态包含：登录页入口、未刮、部分刮开、职业线索、四区域、完整揭晓、新/重复收藏、图鉴筛选、原画详情、展柜、系统返回、退出重启、窄屏明暗主题。采用组件测试与实际 Android APK 截图检查，无联网边界变更，不要求双设备 E2E。
