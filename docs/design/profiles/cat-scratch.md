...
可见返回和 Android Back 共用保存状态检查。正常返回保留收藏；存储错误时提示重试，明确确认后才可放弃未保存进度。结果内联显示，详情用可关闭底部弹层。规则入口和展柜已删除。图鉴与玩家收藏列表允许纵向滚动。

验收状态包含：登录页入口、未刮、部分刮开、未中奖、分组浏览、完整揭晓、新/重复收藏、图鉴筛选、原画详情、玩家列表/他人图鉴、默认公开/自动同步、加载失败/重试、系统返回、退出重启、窄屏明暗主题。采用组件测试与实际 Android APK 截图检查，增加收藏 HTTP 边界，以多账号 HTTP 测试和单 Android 真实客户端验证，不要求对战双设备 E2E。
