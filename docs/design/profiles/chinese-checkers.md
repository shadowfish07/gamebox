...
- 顶部玩家条显示本地先后手、当前轮次与对手在线状态。重连期间不沿用过期的在线状态。
...
上线前至少检查：初始连接，黑/白本地视角，选中与多终点高亮，转向连跳，提交中路径，服务端接受/拒绝，重连保留棋盘，对手在线状态，认输确认，目标营获胜与返回大厅。截图验收必须来自实际构建的目标运行时；静态 HTML 方案只记录交互决策，不是上线验收证据。
